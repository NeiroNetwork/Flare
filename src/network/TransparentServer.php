...
class TransparentServer extends Server{
	 ...
	 * @var Closure[]
	 */
	private array $rawDatagramHandler = [];
	 ...
	 */
	private SnoozeAwarePthreadsChannelWriter $nackWriter;

	public function __construct(int $serverId, \Logger $logger, Socket $socket, int $maxMtuSize, ProtocolAcceptor $protocolAcceptor, ServerEventSource $eventSource, ServerEventListener $eventListener, SnoozeAwarePthreadsChannelWriter $nackWriter, ExceptionTraceCleaner $traceCleaner){
	...
	public function registerRawDatagramHandler(Closure $handler) : void{
		Utils::validateCallableSignature(function(Packet $packet, InternetAddress $address) : bool{
			return false;
		}, $handler);

		$this->rawDatagramHandler[] = $handler;
	}
			...
			if($session !== null){
				...
				if(($header & Datagram::BITFLAG_VALID) !== 0){
					...
					if($packet instanceof NACK){
						$this->nackWriter->write(
							chr(strlen($address->getIp())) .
							$address .
							Binary::writeShort($address->getPort())
						);
					}
					// --------------------------------------------
